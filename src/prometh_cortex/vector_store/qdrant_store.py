...
class QdrantVectorStore(VectorStoreInterface):
            ...
            for result in search_results:
                ...
                formatted_result = {
                    ...
                    "source_file": payload.get(
                        "file_path", payload.get("document_id", "Unknown")
                    ),
                }
    ...
    def delete_documents_except_source(self, excluded_source: str) -> int:
        ...
        Uses Qdrant's filter-based deletion to preserve docs with
        source_type == excluded_source (e.g., "prmth_memory").
        ...
        try:
            # Build filter: delete everything where source_type != excluded_source
            # Using must_not to exclude the preserved source
            delete_filter = Filter(
                must_not=[
                    FieldCondition(
                        key="source_type", match=MatchValue(value=excluded_source)
                    )
                ]
            )

            # Count documents before deletion
            count_before = self.client.count(collection_name=self.collection_name).count

            # Delete non-excluded documents
            self.client.delete(
            ...
            raise RuntimeError(
                f"Failed to delete documents except {excluded_source}: {e}"
            )
    ...
    def _wait_for_collection_ready(self, timeout: int = 30) -> None:
        ...
        raise RuntimeError(
            f"Collection {self.collection_name} not ready after {timeout} seconds"
        )
